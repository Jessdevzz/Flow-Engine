...
-- falling stuff
local function land()
	if (globals.falldistance < -10 and globals.rolling == false) or globals.falldistance < -50 then
		canbob = false
		Humanoid.Health = Humanoid.Health - globals.falldistance
		wait(1.2)
		canbob = true
	end
end
...
bobbing = game:GetService("RunService").RenderStepped:Connect(function(deltaTime)
	if canbob == true then
		...
		if deltaTime > 1.5 then
			...
			func1 = lerp(func1, math.cos(tick() * 0.5 * math.random(5, 7.5)) * (math.random(2.5, 10) / 100) * deltaTime, 0.25 * deltaTime)
			func2 = lerp(func2, math.cos(tick() * 0.5 * math.random(2.5, 5)) * (math.random(1, 5) / 100) * deltaTime, 0.25 * deltaTime)
		end
		...
		func3 = lerp(func3, math.clamp(UserInputService:GetMouseDelta().X, -2.5, 2.5), 0.5 * deltaTime)
		func4 = lerp(func4, math.sin(tick() * int) / 5 * math.min(1, int2 / 10), 0.5 * deltaTime)
